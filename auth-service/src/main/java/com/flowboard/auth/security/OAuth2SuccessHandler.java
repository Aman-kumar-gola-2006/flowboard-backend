...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, 
        ...
        if (rawEmail == null) {
            ...
            response.sendRedirect("http://3.110.61.209.nip.io:4200/login?error=oauth2");
            return;
        }
        ...
        if (!user.getIsActive()) {
            response.sendRedirect("http://3.110.61.209.nip.io:4200/login?error=suspended");
            return;
        }
        ...
        response.sendRedirect("http://3.110.61.209.nip.io:4200/oauth2/callback?token=" + token + "&name=" + finalName + "&id=" + user.getId());
    }
}
